...
const defaultChannels: Channel[] = [
  {
    ...
    description: "Electronic notification of new births",
    url_pattern: "/birth-declaration",
    method: "POST",
    protocol: "HTTP/REST",
    routes: 3,
    enabled: true,
  },
  {
    id: 2,
    name: "Patient Lookup",
    description: "Master Patient Index cross-referencing",
    url_pattern: "/mpi-search",
    method: "GET",
    protocol: "HTTP/REST",
    routes: 1,
    enabled: true,
  ...
  {
    id: 3,
    name: "Lab Results (Legacy)",
    description: "TCP based HL7 receiver",
    url_pattern: "tcp://7000",
    method: "N/A",
    protocol: "HL7 v2.x",
    routes: 2,
    enabled: false,
  },
  {
    ...
    description: "Global registry sync",
    url_pattern: "/immunization/update",
    method: "POST",
    protocol: "HTTP/REST",
    routes: 2,
    enabled: true,
  },
  {
    ...
    description: "E-pharmacy integration gateway",
    url_pattern: "/prescriptions",
    method: "POST",
    protocol: "HTTP/REST",
    routes: 1,
    enabled: true,
  },
  {
    ...
    description: "Real-time trauma unit alerting",
    url_pattern: "/emergency/admit",
    method: "PUT",
    protocol: "HTTP/REST",
    routes: 1,
    enabled: true,
...
const defaultActivity = [
  ...
  },
  {
    id: 2,
    user: "Automated Hook",
    action: "updated routing logic for",
    target: "/birth-declaration",
    time: "5 hours ago",
    icon: "sync",
    color: "bg-tertiary",
  },
  {
    ...
    action: "disabled legacy channel",
    target: "tcp://7000",
    ...
    <div className="animate-slide-in">
      <section className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-gutter">
        <div>
          ...
          <p className="text-body-lg text-on-surface-variant mt-1">
            Configure routing channels for data exchange (OpenHIM-compatible)
          </p>
        </div>
        <div className="flex gap-4">
          <button className="px-6 py-2.5 bg-primary text-white rounded-lg font-bold flex items-center gap-2 hover:opacity-90 active:scale-95 transition-all shadow-md">
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container border-b border-outline-variant">
                ...
                <th className="px-6 py-3 text-label-md text-on-surface-variant uppercase tracking-wider text-center">
                  Routes
                </th>
                <th className="px-6 py-3 text-label-md text-on-surface-variant uppercase tracking-wider">
                  Status
            ...
            <tbody className="divide-y divide-outline-variant">
              {filtered.map((ch) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <code className="text-code-sm bg-surface-container p-1 rounded">
                      {ch.url_pattern}
                    </code>
                  </td>
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 font-bold text-xs rounded ${methodColors[ch.method] || "bg-surface-container-highest text-on-surface-variant"}`}
                    >
                      {ch.method}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <label className="relative inline-flex items-center cursor-pointer">
                      ...
                      <span
                        className={`ml-3 text-label-md font-bold ${ch.enabled ? "text-primary" : "text-on-surface-variant"}`}
                      >
        ...
        <div className="px-6 py-4 border-t border-outline-variant flex items-center justify-between bg-surface-bright">
          <p className="text-label-md text-on-surface-variant">
            Showing {filtered.length} of {channels.length} results
          </p>
        </div>
      </div>

      <section className="mt-margin-desktop">
        <h4 className="text-headline-md font-bold mb-6">
        ...
        <div className="relative pl-8 space-y-8 before:content-[''] before:absolute before:left-[11px] before:top-2 before:bottom-0 before:w-px before:bg-outline-variant">
          {defaultActivity.map((a) => (
            <div key={a.id} className="relative group">
              <div
                className={`absolute -left-8 top-1.5 w-6 h-6 rounded-full ${a.color} flex items-center justify-center ring-4 ring-surface z-10`}
              >
              ...
              <div>
                <p className="text-body-md">
                  ...
                  <span className="font-code-sm text-primary">{a.target}</span>
                </p>
                <time className="text-label-md text-on-surface-variant opacity-60">
                  {a.time}
                </time>
              </div>
            </div>
