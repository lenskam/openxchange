...
const defaultMappings = [
  {
    id: 1,
    name: "Clinical_KPI_Map_v2",
    type: "Variable",
    workflow: "Core Health Monitoring",
    records: 1240,
  ...
  {
    id: 2,
    name: "North_Region_Orgs",
    type: "Org Unit",
    workflow: "Regional Consolidation",
    records: 42,
  ...
  {
    id: 3,
    name: "Standard_Response_Options",
    type: "Option",
    workflow: "Patient Survey Sync",
    records: 18,
  ...
  {
    id: 4,
    name: "DHIS2_DE_Mapping",
    type: "Variable",
    workflow: "Global Reporting",
    records: 5612,
  ...
  return (
    <div className="animate-slide-in">
      <div className="flex justify-between items-start mb-8">
        <div>
          ...
          <p className="text-body-lg text-on-surface-variant mt-1">
            Manage variable, org unit, option, and date format mappings
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-gutter mb-10">
        {(["Variable", "Org Unit", "Option", "Date Format"] as const).map(
          (type) => (
            <div
              key={type}
              className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 flex flex-col justify-between hover:-translate-y-1 transition-transform"
            >
              <div className="flex justify-between items-start">
                <span
                  className={`material-symbols-outlined p-2 rounded-lg ${type === "Variable" ? "text-primary bg-primary-fixed" : type === "Org Unit" ? "text-tertiary bg-tertiary-fixed" : type === "Option" ? "text-secondary bg-secondary-fixed" : "text-outline bg-surface-container"}`}
                >
                  {typeIcons[type]}
                </span>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container-low">
                <th className="px-6 py-4 text-label-md text-on-surface-variant uppercase">
                  Name
                </th>
                <th className="px-6 py-4 text-label-md text-on-surface-variant uppercase">
                  Type
                ...
                <th className="px-6 py-4 text-label-md text-on-surface-variant uppercase text-right">
                  Actions
                </th>
              </tr>
            </thead>
                ...
                >
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      <div
                        className={`h-8 w-8 rounded ${m.color} flex items-center justify-center`}
                      >
                        <span
                          className={`material-symbols-outlined ${m.iconColor} text-[18px]`}
                        >
                          {m.icon}
                        </span>
                      </div>
                  ...
                  <td className="px-6 py-5">
                    <span
                      className={`px-2.5 py-1 rounded-full text-[11px] font-bold uppercase ${typeBadge[m.type] || "bg-surface-container text-on-surface-variant"}`}
                    >
                      {m.type}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-5 font-mono text-code-sm">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-2">
                      <div
                        className={`h-6 w-6 rounded-full ${m.color} flex items-center justify-center text-[10px] font-bold`}
                      >
                        {m.initials}
                      </div>
      ...
      >
        <MenuItem
          onClick={() => {
            setSnack({ message: "Download started", severity: "success" });
            setMenuAnchor(null);
          }}
        >
          Download
        ...
        <MenuItem
          onClick={() => {
            setSnack({ message: "Mapping deleted", severity: "success" });
            setMenuAnchor(null);
          }}
        >
          Delete
